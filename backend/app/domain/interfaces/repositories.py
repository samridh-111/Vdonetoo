...
class AudioFileRepository(Protocol):
    ...
    async def average_generation_time_ms(self, sample_size: int = 50) -> float | None:
        """Average generation_time_ms across the most recent completed audio
        files, globally (not batch-scoped) -- used for pre-generation time
        estimates before any job in the new batch has run yet."""
        ...
